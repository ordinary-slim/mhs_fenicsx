...
def run(case_name="case"):
    ...
    nx = np.round((box[3]-box[0]) * point_density).astype(np.int32)
    ny = np.round((box[4]-box[1]) * point_density).astype(np.int32)
    nz = np.round((box[5]-box[2]) * point_density).astype(np.int32)
