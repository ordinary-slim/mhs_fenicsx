...
def interpolate(projectedFunction,
                targetSpace,):
    nmmid = dolfinx.fem.create_nonmatching_meshes_interpolation_data(
                                 targetSpace.mesh,
                                 targetSpace.element,
                                 projectedFunction.ufl_function_space().mesh,
                                 padding=1e-5,)
    interpolation = dolfinx.fem.Function(targetSpace)
    interpolation.interpolate(projectedFunction, nmm_interpolation_data=nmmid)
...
def getMask(size, indices, dtype=np.int32):
...
def interpolate_dg_at_facets(f,facets,targetSpace,bb_tree_ext,
    ...
    for idx in range(total_facet_count):
        ...
        parent_els_ext = geometry.compute_colliding_cells(ext_domain, candidate_parents_ext, global_interface_midpoints[idx,:])
        parent_els_ext = parent_els_ext.array[np.flatnonzero( ext_activation_tag.values[ parent_els_ext.array] ) ]
        if len(parent_els_ext)>0:
            idx_owner_el = parent_els_ext[0]
    ...
    for idx, ifacet in enumerate(facets):
        ...
        flat_idx    = parent_els[0]*interpolated_f.ufl_shape[0]
...
class Problem:
    def __init__(self, mesh, name="case"):
        ...
        self.bb_tree = geometry.bb_tree(self.domain,self.dim)
        self.setActivation()

        self.u = fem.Function(self.v, name="uh")

    def setActivation(self, active_els=None):
        ...
        self.active_els_tag = mesh.meshtags(self.domain, self.dim,
                                            ...
                                            getMask(self.num_cells, active_els),)
        self.domain.topology.create_connectivity(self.dim,self.dim)
        self.active_els_func= indices_to_function(self.dg0_bg,active_els,self.dim)
        self.active_els_func.name = "active_els"
        ...
        self.bfacetsTag  = mesh.meshtags(self.domain, self.dim-1,
                                         np.arange(self.num_facets, dtype=np.int32),
                                         getMask(self.num_facets, locate_active_boundary( self.domain, active_els ), dtype=np.int32),
                                         )

    def getActiveInExternal(self, p_ext:Problem ):
        '''
        ...
        nmmid = dolfinx.fem.create_nonmatching_meshes_interpolation_data(
                                     ...
                                     padding=1e-5,)
        active_dofs_ext_func_self = dolfinx.fem.Function(self.v,
    ...
    def findGamma(self, p_ext:Problem):
        gammaFacets = []
        active_dofs_func = self.getActiveInExternal( p_ext )
        ...
        for ifacet in self.bfacetsTag.find(1):
            local_con = con_facet_nodes.links(ifacet)
            all_nodes_active = True
            for inode in local_con:
                if not(active_dofs_func.x.array[inode]):
        ...
        self.gammaFacets = mesh.meshtags(self.domain, self.dim-1,
                                         np.arange(self.num_facets, dtype=np.int32),
                                         getMask(self.num_facets, gammaFacets),)
        self.gammaNodes = indices_to_function(self.v,
                                         self.gammaFacets.find(1),
                                         self.dim-1,)
        self.gammaNodes.name = "gammaNodes"



    def setDirichletBc(self, func, bdofs=None, bfacetsTag=None):
        if bdofs is None:
            if bfacetsTag is None:
                bfacetsTag = self.bfacetsTag
                bdofs = fem.locate_dofs_topological(self.v, self.dim-1, bfacetsTag.find(1),)
        u_bc = fem.Function(self.v)
        u_bc.interpolate(func)
        self.bc = fem.dirichletbc(u_bc, bdofs)

    def getGammaIntegralEntities(self, gammaIndices=None):
        gammaIntegralEntities = []
        ...
        for ifacet in gammaIndices:
            ...
            gammaIntegralEntities.append(active_cells[0])
            gammaIntegralEntities.append(local_index[0])
        return gammaIntegralEntities

    def setForms(self, rhs, potential_ext=None, flux=None, compileForms=True):
        ...
        if (potential_ext is not None) or (flux is not None):
            gammaIntegralEntities = self.getGammaIntegralEntities()
            # Custom measure
            ...
            if (potential_ext is not None):
                potential = interpolate(potential_ext,self.v)
                self.l_ufl += +ufl.inner(n, ufl.grad(potential)) * v * dS(8)
            elif (flux is not None):
                self.l_ufl += +ufl.inner(n, flux) * v * dS(8)
        if compileForms:
            self.compileForms()

    def compileForms(self):
    ...
    def assemble(self):
        ...
        multiphenicsx.fem.petsc.apply_lifting(self.L, [self.a_compiled], [[self.bc]], restriction=self.restriction,)
        self.L.ghostUpdate(addv=petsc4py.PETSc.InsertMode.ADD, mode=petsc4py.PETSc.ScatterMode.REVERSE)
        multiphenicsx.fem.petsc.set_bc(self.L,[self.bc],restriction=self.restriction)
    ...
    def writepos(self, extra_funcs=[]):
        funcs = [self.u, self.active_els_func]
        funcs.extend(extra_funcs)
        with io.VTKFile(self.domain.comm, f"out/results_{self.name}.pvd", "w") as ofile:
            ofile.write_mesh(self.domain)
            ofile.write_function(funcs)

    def write_bmesh(self):
        bmesh = dolfinx.mesh.create_submesh(self.domain,self.dim-1,self.bfacetsTag.find(1))[0]
