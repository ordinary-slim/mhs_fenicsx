...
def main(input_file, writepos=True):
    ...
    el_size = get_el_size(els_per_radius)
    point_density = np.round(1/el_size).astype(np.int32)
    ...
    p_fixed = problem.Problem(domain, params, name=pname)
    ...
    if driver_type == "staggered":
        ...
        Driver = MonolithicRRDriver
        gamma_coeff1 = 1.0
        gamma_coeff2 = 1.0
    driver = Driver(p_fixed, p_moving, gamma_coeff1, gamma_coeff2)
